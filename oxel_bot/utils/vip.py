...
def sync_user_loyalty_and_vip(db, user_id: int) -> dict:
    ...
    order_earned_pts = total_spend // 4  # 1 point per 4 ETB spent
    ...
    if total_spend >= 15000 or synced_points >= 3750:
        tier = "Gold 🥇"
        next_tier = "MAX VIP"
        needed = 0
        perk = "🎁 Free Custom Engraving (+400 ETB waived) + 1.5x Points Bonus!"
    elif total_spend >= 5000 or synced_points >= 1250:
        tier = "Silver 🥈"
        next_tier = "Gold 🥇"
        ...
        perk = "🎁 Standard Loyalty Rewards (1 pt per 4 ETB)"
